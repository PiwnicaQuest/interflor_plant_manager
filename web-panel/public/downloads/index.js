...
async function setupSystemTray(server) {
    ...
    try {
        ...
        const SysTray = (await Promise.resolve().then(() => __importStar(require("systray2")))).default;
...
async function main() {
    ...
    const printers = await printerDetector_1.printerDetector.getPrinters(true);
    ...
    // Start server
    const server = new server_1.PrintBrokerServer();
    try {
        await server.start();
        // Setup system tray
