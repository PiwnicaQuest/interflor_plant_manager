...
export function ProformaTemplate({ data, sellerInfo = defaultSellerInfo }: ProformaTemplateProps) {
  ...
  return (
    <div className="proforma-template bg-white text-black p-8 max-w-[210mm] mx-auto font-sans text-sm">
      <style>{`
        @media print {
          @page { size: A4; margin: 10mm; }
          body { margin: 0; padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          .proforma-template { max-width: 100% !important; padding: 0 !important; }
        ...
        <table className="w-full border-collapse text-xs">
          ...
          <tfoot>
            <tr className="bg-gray-100 font-bold">
              ...
              <td className="border border-gray-300 p-2 text-right">{(Number(data.subtotalNet) || 0).toFixed(2)} zl</td>
              <td className="border border-gray-300 p-2 text-right">{(Number(data.totalVat) || 0).toFixed(2)} zl</td>
              <td className="border border-gray-300 p-2 text-right text-purple-600">{(Number(data.totalGross) || 0).toFixed(2)} zl</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div className="grid grid-cols-2 gap-8 mb-6">
        <div>
          ...
          {data.paidAmount > 0 && data.paidAmount < data.totalGross && (
            <p className="text-sm">Zaplacono: <span className="font-semibold">{(Number(data.paidAmount) || 0).toFixed(2)} zl</span></p>
          )}
        ...
        <div className="flex justify-end">
          <div className="bg-purple-50 p-4 rounded-lg border-2 border-purple-200 w-64">
            <h3 className="text-xs font-semibold text-gray-500 uppercase mb-2">Do zaplaty</h3>
      ...
      {/* Info about Pro Forma */}
      <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
        <p className="text-xs text-purple-700">
          Pro forma nie jest dokumentem ksiegowym. Stanowi wstepna kalkulacje kosztow i nie uprawnia do odliczenia podatku VAT.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-8 mt-12 pt-6">
        <div className="text-center">
          <div className="border-t border-gray-400 pt-2 mt-12">
            <p className="text-xs text-gray-500">Podpis osoby upowaznonej do wystawienia</p>
          </div>
        </div>
        <div className="text-center">
          <div className="border-t border-gray-400 pt-2 mt-12">
