  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && order) {
        ...
        await api.createOrder({ customerId: selectedCustomerId!, items: items.map(item => ({ productId: item.productId!, quantity: item.quantity, palletCount: item.palletCount, unitsPerPallet: item.unitsPerPallet })), customerNotes: customerNotes || undefined });
      }
      onSave();
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="block text-sm font-medium text-gray-700">Klient <span className="text-red-500">*</span></label>
