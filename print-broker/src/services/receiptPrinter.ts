...
function extractIp(port: string | null): string | null {
  if (!port) return null;
  // Direct IP
  if (/^(\d{1,3}\.){3}\d{1,3}$/.test(port)) return port;
...
// =============================================
async function sendRawViaUsb(data: Buffer, printerName: string): Promise<boolean> {
...
async function sendRawViaWinspool(data: Buffer, printerName: string): Promise<boolean> {
  if (process.platform !== "win32") return false;
...
`;
    ...
    try { fs.unlinkSync(ps1WinPath); } catch {}
...
// =============================================
async function sendToPrinter(data: Buffer, printerName: string): Promise<void> {
  ...
  // 2. Try USB raw write
  if (await sendRawViaUsb(data, printerName)) return;

  // 3. Try Windows RAW print API (winspool.drv)
  if (await sendRawViaWinspool(data, printerName)) return;
...
// =============================================
function buildEscPosRaster(bitmapData: number[], widthDots: number, heightDots: number, copies: number): Buffer {
  const bytesPerLine = Math.ceil(widthDots / 8);
...
export async function printReceipt(request: PrintRequest, printerName: string): Promise<void> {
  ...
  if (request.contentType === "html") {
    ...
    let buffer: Buffer;
    try { buffer = Buffer.from(request.content, "base64"); }
    catch { buffer = Buffer.from(request.content, "utf-8"); }
    if (copies > 1) {
      const bufs: Buffer[] = [];
      for (let i = 0; i < copies; i++) bufs.push(buffer);
      ...
      if (!pdfPrinted) {
        try {
          ...
          fs.writeFileSync(ps1Path,
            "Start-Process -FilePath '" + safePdf + "' -Verb PrintTo -ArgumentList '" + safePrinter + "' -Wait\n"
          );
...
// =============================================
async function printReceiptViaBitmap(
  page: any,
  printerName: string,
  paperWidthMm: number,
  copies: number
  ...
  const bitmap: { data: number[], width: number, height: number } = await page.evaluate(
    async (imgB64: string, targetW: number) => {
      return new Promise<{ data: number[], width: number, height: number }>((resolve) => {
        const img = new Image();
        img.onload = () => {
