...
export class InventoryController {
  ...
  static async create(req: AuthRequest, res: Response) {
    try {
      ...
      if (data.purchasePricePln > 0) {
        calculatedPrices = SettingsModel.calculatePrices(data.purchasePricePln, costPercentage, marginPercentage);
      }
  ...
  static async update(req: AuthRequest, res: Response) {
    try {
      ...
      // Sprawdź czy zmieniono inne ceny ręcznie (nie basePriceGross)
      const isOtherPriceManualUpdate =
        data.pricePlus !== undefined ||
      ...
      if (basePriceChanged && !isOtherPriceManualUpdate) {
      ...
      if (data.purchasePricePln !== undefined &&
          ...
          !basePriceChanged && !isOtherPriceManualUpdate) {
        const pricingSettings = await SettingsModel.getPricingSettings();
        const { costPercentage, marginPercentage } = pricingSettings;

        if (data.purchasePricePln > 0) {
          const calculatedPrices = SettingsModel.calculatePrices(data.purchasePricePln, costPercentage, marginPercentage);
          updatedData = {
            ...updatedData,
