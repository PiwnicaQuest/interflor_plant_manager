...
export interface CompanySettings {
  ...
  bankAccount?: string;
  bankSwift?: string;
  invoiceComment?: string;  // Komentarz wyświetlany na fakturach
}
...
export class SettingsModel {
  ...
  static async getCompanySettings(): Promise<CompanySettings> {
    ...
    const invoiceComment = await this.getSetting('company_invoice_comment');
  ...
  static async updateCompanySettings(settings: CompanySettings): Promise<CompanySettings> {
    ...
    await this.upsertSetting('company_invoice_comment', settings.invoiceComment || '', 'Komentarz na fakturach');
   ...
   */
  static calculatePrices(purchasePriceNet: number, costPercentage: number, marginPercentage: number, vatRate: number = 8.0) {
    // 1. Cena+ = Cena zakupu × (1 + % kosztów / 100)
    const pricePlus = purchasePriceNet * (1 + costPercentage / 100);

    // 2. Cena podstawowa = Cena+ × (1 + vatRate%) × (1 + % marży / 100)
    const basePriceGross = pricePlus * (1 + vatRate / 100) * (1 + marginPercentage / 100);
